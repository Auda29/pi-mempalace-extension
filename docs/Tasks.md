...
Stand nach `0.1.4`: Einige High-Priority- und Robustheits-Punkte wurden bereits vorgezogen und sind hier entsprechend als `done` markiert.

| # | Prio | Status | Bereich | Task |
|---|---|---|---|---|
| 5  | Mittel  | open | Resolver | `MEMPALACE_PYTHON` / `MEMPALACE_VENV` direkt aus `env` werden ohne `expandHomeDir` gelesen. `~/...` sollte defensiv direkt im Resolver expandiert werden. |
...
| 7  | Hoch    | open | Doctor | Fehlgeschlagene Tool- oder Event-Registrierungen loggen aktuell nur `warn`. Eigene Doctor-Checks fuer registrierte Tools und Pi-Hooks ergaenzen. |
| 8  | Hoch    | open | CLI-Wrapper | Bei `result.timedOut` wird noch eine generische Meldung benutzt. Vorhandener `stderr` sollte in Timeout-Meldungen einbezogen werden. |
...
| 10 | Niedrig | open | Tools | `mempalace_mine` / `mempalace_init` fallen ohne `path` und `cwd` auf `process.cwd()` der Extension zurueck. Das sollte in den Tool-Descriptions explizit dokumentiert werden. |
...
| 14 | Niedrig | open | Hosts | Host-Helfer sollten neben synchronen Fehlern auch rejected Promises defensiv behandeln. |
