...
import { getPiRegistrationHealth } from "./pi-health.js";
...
export async function runDoctor(options) {
    ...
    const registrationHealth = options.registrationHealth ?? getPiRegistrationHealth();
...
async function checkPiRegistrations(health) {
    ...
    const hasAnyRegistration = health.toolsRegistered > 0 || health.hooksRegistered > 0;
    if (!hasAnyRegistration && !hasToolFailures && !hasHookFailures) {
        return {
            id: "pi-registrations",
            title: "Pi tools and hooks registered",
            status: "warn",
            summary: "No Pi tool or hook registrations have been observed in this process yet.",
    ...
    if (hasToolFailures || hasHookFailures) {
        return {
            ...
            title: "Pi tools and hooks registered",
            status: "fail",
            summary: "Some Pi tools or hooks failed to register in the current process.",
