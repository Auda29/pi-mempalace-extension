...
async function resolveRuntimeForDoctor(
  config: MempalaceConfig,
  env: NodeJS.ProcessEnv,
  logger?: Logger,
...
async function checkPythonRuntime(
  ...
  try {
    const { stdout, stderr } = await execFile(
      runtime.exe,
      [...getPythonLauncherArgs(runtime), "--version"],
      {
...
async function checkMempalacePackage(
  runtime: ResolvedRuntime | null,
): Promise<DoctorCheck> {
  if (!runtime || runtime.kind !== "python") {
    return {
...
async function checkMempalaceVersionCommand(
  runtime: ResolvedRuntime | null,
  env: NodeJS.ProcessEnv,
  config: MempalaceConfig,
): Promise<DoctorCheck> {
...
async function checkWindowsUtf8(
  runtime: ResolvedRuntime | null,
  env: NodeJS.ProcessEnv,
  config: MempalaceConfig,
): Promise<DoctorCheck> {
...
function buildPythonFix(config: MempalaceConfig): string {
